...
import { setDistrictsEnd, defaultDistrictsEnd } from '../../reducers/districtsEnd';
import { ChangePlaceName } from '../../reducers/infoPlaceName';

function FormEndPlace(props) {
	...
	const HandleChange = (e) => {
		...
		if (val.length > 5) {
			...
			if(name.length === 7){
				...
				dispatch(setDistrictsEnd(Place[val].districts));
			}else {
				dispatch(ChangePlace({ [name]: val }));
				dispatch(ChangePlaceName({ [name]: Place[InfoPlace.endCity].districts[val].name}));
			}	
		}
		setValue({ [name]: val });
	}
